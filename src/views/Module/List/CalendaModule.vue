...
<div class="calendar">
    ...
    <tbody>
        ...
        <!-- <tr v-for="(week, weekIndex) in weeks" :key="weekIndex">
        <td v-for="(day, dayIndex) in week" :key="`${day.date.getTime()}_${dayIndex}`" :class="{ today: isToday(day.date) }">{{ day.day }}</td>
        </tr> -->
    </tbody>
    </table>
</div>
...
.today {
    /* font-weight: bold; */
    /* color: red; */
    background: url(@/assets/img/Icon/select.png) 50% no-repeat;
